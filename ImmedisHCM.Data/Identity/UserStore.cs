...
namespace ImmedisHCM.Data.Identity
{
    ...
    UserStoreBase<TUser, TRole, Guid, WebUserClaim, WebUserRole, WebUserLogin, WebUserToken, WebRoleClaim>,
    IProtectedUserStore<TUser> where TUser : WebUser where TRole : WebRole
    {
        ...
        private IQueryable<WebUserClaim> UserClaims => session.Query<WebUserClaim>();

        private IQueryable<WebUserRole> UserRoles => session.Query<WebUserRole>();

        private IQueryable<WebUserLogin> UserLogins => session.Query<WebUserLogin>();

        private IQueryable<WebUserToken> UserTokens => session.Query<WebUserToken>();
        ...
        protected override async Task<WebUserRole> FindUserRoleAsync(
            Guid userId,
            Guid roleId,
            CancellationToken cancellationToken
        )
        ...
        protected override async Task<WebUserLogin> FindUserLoginAsync(
            Guid userId,
        ...
        protected override async Task<WebUserToken> FindTokenAsync(
            TUser user,
            string loginProvider,
            string name,
            CancellationToken cancellationToken
        ...
        protected override async Task AddUserTokenAsync(WebUserToken token)
        {
            ThrowIfDisposed();
            if (token == null)
            {
